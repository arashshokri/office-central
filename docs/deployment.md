...
The installer validates the FQDN, generates application/database/Redis/Ed25519 secrets, builds the stack, migrates PostgreSQL, and securely prompts for the first administrator. Configure the existing Nginx UI or host proxy to send `my.ponet.ir` to `http://127.0.0.1:8787` as described in [host Nginx proxy](host-nginx-proxy.md). TLS issuance and renewal remain in that existing proxy.
