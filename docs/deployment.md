...
Use Debian 12 or Ubuntu 24.04 with Docker Engine, Compose v2, Git, OpenSSL, at least 2 GB RAM, and sufficient disk for PostgreSQL plus immutable packages. Office Central binds directly to port 80. Make sure that port is available and create the DNS `A`/`AAAA` records before installation.
...
The installer validates the FQDN, generates application/database/Redis/Ed25519 secrets, builds the stack, migrates PostgreSQL, and securely prompts for the first administrator. The panel is served directly on port 80; point `panel.ponet.ir` to the server running this stack.
