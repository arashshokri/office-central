...
Laravel/PHP-FPM serves the bilingual admin panel and versioned Agent API. PostgreSQL is the source of truth, Redis backs cache, queues and rate limits, Nginx serves the application, package ZIPs live on private persistent storage, and Ed25519 signs leases independently of TLS. Compose runs `app`, `nginx`, `postgres`, `redis`, `queue-worker`, and `scheduler` with persistent volumes.
...
Use `centralctl.sh` for install, lifecycle, status, logs, doctor, backup, restore, tagged update, and rollback. See [commands](docs/centralctl.md), [backup/restore](docs/backup-restore.md), and [updates](docs/update-rollback.md).
